...
# Lower the voice by this many semitones (0 = unchanged).
PITCH = float(os.environ.get("PITCH_SEMITONES", "-1"))
...
def write(seg, dest):
    ...
    ratio = 2 ** (PITCH / 12)
    chain = f"asetrate={int(SR * ratio)},aresample={SR},atempo={TEMPO / ratio:.5f}" if PITCH else f"atempo={TEMPO}"
    subprocess.run(FFMPEG + ["-i", tmp, "-af", chain, "-ac", "1", "-b:a", "128k", dest], check=True)
    os.remove(tmp)
